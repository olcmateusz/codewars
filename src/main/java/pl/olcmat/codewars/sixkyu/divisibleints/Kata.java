package pl.olcmat.codewars.sixkyu.divisibleints;
...
public class Kata
{
  public static int getCount(int n)
  {
    ...
    List<String> listOfAllDividers = new ArrayList<String>();

    //filling list with all possible dividers
    for (int i =1; i<number.length();i++){
      for (int j=0;j+i<=number.length();j++){
        //check if the substring is equal to zero to avoid division by 0
        if (Integer.valueOf(number.substring(j,i+j)) != 0) listOfAllDividers.add(number.substring(j,j+i));
      }
    }

    int divisionWithoutRemainderCounter = 0;
    for (String element: listOfAllDividers) {
      if (n % Integer.valueOf(element) == 0) divisionWithoutRemainderCounter++;
    }
    return divisionWithoutRemainderCounter;
  }
}